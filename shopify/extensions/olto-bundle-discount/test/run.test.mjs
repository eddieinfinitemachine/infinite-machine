...
const CARGO = [SIDEWALLS, DOCK, PHONE, CHARGER_BAG, RACK, BASKET, SIDE_PLATE, ACC_PLATE, STAND];
const MAX = [...CARGO, BOTTLE, SUPER];

test("full Commuter set -> -$102", () => {
  const out = run(cart([OLTO, ...COMMUTER]));
  assert.equal(amount(out), "102.00");
  assert.equal(candidate(out).message, "Olto Commuter bundle");
  assert.equal(candidate(out).targets.length, COMMUTER.length);
});

test("full Cargo set -> -$322", () => {
  const out = run(cart([OLTO, ...CARGO]));
  assert.equal(amount(out), "322.00");
  assert.equal(candidate(out).message, "Olto Cargo bundle");
});

test("full Max set -> -$418, the larger tier only (Max contains Cargo)", () => {
  const out = run(cart([OLTO, ...MAX]));
  assert.equal(amount(out), "418.00");
...
test("Max cart must not fall through to Cargo (would under-discount by $96)", () => {
  assert.notEqual(amount(run(cart([OLTO, ...MAX]))), "322.00");
});
...
test("partial sets never leak", () => {
  ...
  assert.deepEqual(run(cart([OLTO, ...CARGO.slice(0, 8)])).operations, [], "Cargo minus the stand");
});
